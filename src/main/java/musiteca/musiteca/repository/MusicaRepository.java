...
public interface MusicaRepository extends JpaRepository<Musica, String>{
    ...
    @Query(value="Select a from Musica a where a.nome=:pnome")
    List<Musica> getMusicasUsuario(@Param("pnome") String usuario);

    @Query(value="Select a from Musica a where a.nome=:pnome AND a.nomeArtist=:partist")
    List<Musica> getMusicasArtista(@Param("pnome") String usuario, @Param("partist") String artista);

    @Query(value="Select a from Musica a where a.nome=:pnome AND a.nomeArtist=:partist AND a.albumNome=:palbum")
